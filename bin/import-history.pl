...
# Load history XML data to SQL tables.
...
my $xml_dir = '../history/',
my $clubs_xml = '../clubs.xml',
my $xml_catalog = '../lax.xmlcatalog';
...
# my $competition = $dbh->prepare( "INSERT INTO semla_competition (`id`,
# 	`name`, `league_id`, `abbrev`, `seq`,`type`,`head_to_head`,`has_history`, `history_page`
# 	) VALUES (?,?,?,?,?,?,?,?,?)");
my $hist_winner = $dbh->prepare( "INSERT INTO semla_hist_winner (`comp_id`,
	`year`, `winner`, `runner_up`, `result`,
	`win_goals`, `lose_goals`, `has_data`
	) VALUES (?,?,?,?,?,?,?,?)");
my $hist_table = $dbh->prepare( "INSERT INTO semla_hist_table (
	`year`, `comp_id`, `position`, `team`, `team_minimal`,
	`played`, `won`, `drawn`, `lost`, `goals_for`, `goals_against`,
	`goal_avg`, `points_deducted`, `points`, `points_avg`, `divider`
	) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
my $hist_cup_draw = $dbh->prepare( "INSERT INTO semla_hist_cup_draw (
	`year`,	`comp_id`, `round`, `match_num`, `team1`,
	`team2`, `team1_goals`, `team2_goals`,
	`result_extra`, `home_team`
  	) VALUES (?,?,?,?,?,?,?,?,?,?)");
my $hist_result = $dbh->prepare( "INSERT INTO semla_hist_result (
	`year`, `match_date`, `comp_id`, `comp_id2`,
	`competition`, `home`, `away`, `home_goals`, `away_goals`,
	`result`, `home_points`, `away_points`, `points_multi`
  	) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
my $hist_remarks = $dbh->prepare( "INSERT INTO semla_hist_remarks ( `comp_id`, `year`, `remarks`) VALUES (?,?,?)");
my $team_abbrev = $dbh->prepare( "INSERT INTO semla_team_abbrev (`team`, `abbrev`) VALUES (?,?)");
...
sub loadWinners {
	...
	foreach my $file (@winnerOnly) {
	    ...
	    foreach my $winner ($dom->findnodes('/results/winner')) {
    		$hist_winner->execute($compId, $winner->{year}, $winner->to_literal(), undef, undef, 0, 0, 0);
		}
	}
	...
	foreach my $file (@multi) {
		...
		foreach my $comp ($dom->findnodes('/results/competitions/comp')) {
			...
			my $compId = getCompetitionId($title,'winner');
			foreach my $winner ($dom->findnodes('/results/season/winner[@compId="' . $comp->{id}. '"]')) {
				$hist_winner->execute($compId, $winner->parentNode->{year}, $winner->to_literal(), undef, undef, 0, 0, 0);
			}
		}
	}
}
	...
	foreach my $league ($dom->findnodes('/league-results/league')) {
		if ($league->{ref}) {
			...
			foreach my $winner ($league->findnodes('winner')) {
	    		...
	    		$hist_winner->execute($compId, $league->{year}, $winner->to_literal(), undef, undef, 0, 0, 0);
			}
		}
	}
}
		...
		foreach my $team ($division->findnodes('team')) {
			...
			if ($pos == 0) {
				if (! $division->{'preliminary'}) {
					$hist_winner->execute($compId, $year, $teamName, undef, undef, 0, 0, 1);
				}
			}
